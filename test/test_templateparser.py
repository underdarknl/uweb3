...
class ParserDirectoryTests(unittest.TestCase):
    ...
    def testAttemptReadNonExistingFile(self):
        parser = templateparser.Parser(path=self.template_dir)
        self.assertRaises(templateparser.TemplateReadError, parser.AddTemplate, 'non_existing_file.html')
...
class TemplateTagBasic(unittest.TestCase):
    ...
    def testSaveTagTemplate(self):
        ...
        result = self.tmpl(template).Parse(
            single=templateparser.HTMLsafestring("<b>a safe</b>")
        )
...
class TemplateTagFunctions(unittest.TestCase):
    ...
    def testNonexistantFuntion(self):
        ...
        self.assertRaises(
            templateparser.TemplateFunctionError, self.parse, template, num=1
        )
...
class TemplateTagFunctionClosures(unittest.TestCase):
    ...
    def testNamedArguments(self):
        ...
        self.assertRaises(
            templateparser.TemplateSyntaxError, self.parse, template, tag=self.tag
        )
...
class TemplateUnicodeSupport(unittest.TestCase):
    ...
    def testUnicodeReplacements(self):
        ...
        expected = "Underdark Web framework, also known as \xb5Web."
        self.assertEqual(self.parse(template, name="\xb5Web"), expected)

    def testUnicodeTagFunction(self):
        """[Unicode] Template functions returning unicode are converted to UTF8"""
        function_result = "No more \N{BLACK HEART SUIT}"
...
class TemplateConditionals(unittest.TestCase):
    ...
    def testElifTagWithFunc(self):
        ...
        parser.RegisterFunction("test", lambda x: len(x))
        template = (
            "{{ if [variable|test] == 4}}foo{{elif [variable|test] == 5}}bar{{ endif}}"
        )
        self.assertEqual(parser.ParseString(template, variable="1234"), "foo")
        self.assertEqual(parser.ParseString(template, variable="12345"), "bar")
    ...
    def testVariableMustBeTag(self):
        ...
        self.assertRaises(
            templateparser.TemplateNameError, self.parse, bad_template, var="foo"
        )
...
class TemplateLoops(unittest.TestCase):
    ...
    def testLoopOnFunctions(self):
        ...
        template = (
            "{{ for item in [mapping|items|sorted] }}" "[item:0]=[item:1] {{ endfor }}"
        )
        ...
        unsorted = "{{ for item in [mapping|items] }} " "[item:0]=[item:1]{{ endfor }}"
    ...
    def testLoopTupleAssignment(self):
        ...
        template = (
            "{{ for key,val in [mapping|items|sorted] }}" "[key]=[val] {{ endfor }}"
        )
        mapping = {"first": 12, "second": 42}
        result = "first=12 second=42 "
        self.assertEqual(self.parse(template, mapping=mapping), result)
    ...
    def testLoopTupleAssignmentMismatch(self):
        ...
        self.assertRaises(
            templateparser.TemplateValueError, self.parse, template, iterator=["eggs"]
        )
        self.assertRaises(
            templateparser.TemplateValueError, self.parse, template, iterator=range(10)
        )
...
class TemplateStringRepresentations(unittest.TestCase):
    ...
    def testTemplateLoop(self):
        ...
        template = (
            "{{ for a, b in [iter|items] }}{{ for c in [a] }}[c]"
            "{{ endfor }}{{ endfor }}"
        )
        self.assertEqual(self.strip(str(self.tmpl(template))), self.strip(template))
...
class TemplateReloading(unittest.TestCase):
    ...
    def testFileBasicReload(self):
        ...
        with open(self.simple, "w") as new_template:
            ...
            time.sleep(0.01)  # short pause so that mtime will actually be different
...
class DictTemplateTagBasic(unittest.TestCase):
    ...
    def testSingleTagTemplate(self):
        """[BasicTag] Templates with basic tags get returned proper"""
        template = "Template with [single] tag"
        output = {"tags": {"[single]": "just one"}, "templatecontent": template}
    ...
    def testSaveTagTemplate(self):
        """[BasicTag] Templates with basic tags get returned properly when replacement is already html safe"""
        template = "Template with just [single] tag"
        output = {"tags": {"[single]": "<b>a safe</b>"}, "templatecontent": template}
    ...
    def testMultiTagTemplate(self):
        ...
        template = "[adjective] [noun] are better than other [noun]."
        output = {
            "tags": {"[noun]": "cows", "[adjective]": "Beefy"},
            "templatecontent": template,
        }
        result = self.tmpl(template, dictoutput=True).Parse(
            noun="cows", adjective="Beefy"
        )
        self.assertEqual(result, output)
    ...
    def testBadCharacterTags(self):
        ...
        replaces = {char: "FAIL" for char in bad_chars}
        replaces["check"] = ".."
        output = {"tags": tags, "templatecontent": template}
    ...
    def testBracketsInsideTag(self):
        ...
        output = {
            "tags": {"[spam]": "opening or ", "[eggs]": "closing brackets"},
            "templatecontent": template,
        }
