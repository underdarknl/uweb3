...
class ConnectionManager(object):
    ...
    def RelevantConnection(self, level=2):
        ...
        con_type = (
            caller_cls._CONNECTOR
            if hasattr(caller_cls, "_CONNECTOR")
            else self.DEFAULTCONNECTIONMANAGER
        )
        if con_type in self.__connections and hasattr(
            self.__connections[con_type], "connection"
        ):
        ...
        try:
            ...
            raise TypeError(
                "No connector for: %r, available: %r, %r"
    ...
    def request(self):
        ...
        raise TypeError(
            'No request could be found in call Stack or no "model" connections are present.'
        )
    ...
    def PostRequest(self):
        ...
        for classname in self.__connections:
            if (
                hasattr(self.__connections[classname], "PERSISTENT")
