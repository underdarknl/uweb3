...
def Cached(maxage=None, verbose=False, handler=None, *t_args, **t_kwargs):
    ...
    def cache_decorator(f):
        def wrapper(*args, **kwargs):
            if not handler:
                raise KeyError(
                    "A handler is required for storing this page into the database."
                )
            create = False
            name = f.__name__
            ...
            try:  # see if we have a cached version thats not too old
                ...
                while (
                    age < maxage
                ):  # as long as there's no output, we should try periodically until we have waited too long
                    time.sleep(sleep)
