...
class BasePageMaker(Base):
    ...
    CACHE_DURATION = MimeTypeDict(
        {"text": 7, "image": 30, "application": 7, "text/css": 7}
    )

    def __init__(self, req, config=None, executing_path=None):
        ...
        try:
            ...
            with open(abs_path, readtype) as staticfile:
                ...
                return response.Response(
                    ...
                    headers={
                        "Expires": expires.strftime(RFC_1123_DATE),
                        "cache-control": "max-age=%d" % (cache_days * 24 * 60 * 60),
...
class DebuggerMixin:
    ...
    def InternalServerError(self, exc_type, exc_value, traceback):
        ...
        error_template = templateparser.FileTemplate(
            os.path.join(os.path.dirname(__file__), self.ERROR_TEMPLATE)
        )
        try:
            return response.Response(
...
class SparseAsyncPages(BasePageMaker):
    ...
    def SparseTemplateProvider(self, content_hash, path):
        ...
        try:
            ...
            return response.Response(
                "This template does not exists", content_type="text/plain", httpcode=404
            )
        if template._template_hash != content_hash:
            return response.Response(
                "This template does not exists anymore. %s %s"
                % (template._template_hash, content_hash),
                content_type="text/plain",
                httpcode=404,
            )
    ...
    def SparseRenderedProvider(self):
        return response.Response(
            templateparser.FileTemplate(
                os.path.join(os.path.dirname(__file__), "sparserenderer.js")
            ),
