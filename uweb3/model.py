    ...
    def __CheckPermissions(self):
        ...
        if not os.access(self.file_location, os.R_OK):
            raise PermissionError(
                f"SettingsManager missing permissions to read file: {self.file_location}"
            )
...
class Record(BaseRecord):
        ...
        if isinstance(cls._PRIMARY_KEY, tuple):
            ...
            return " AND ".join(
                ...
                for field, value in zip(
                    cls._PRIMARY_KEY, connection.EscapeValues(values)
                )
            )
        return "`%s`.`%s` = %s" % (
            cls.TableName(),
    ...
    def Refresh(self: R) -> R:
        ...
        """
        result = self.FromPrimary(self.connection, self[self._PRIMARY_KEY])
        self.__dict__.update(result._record)
        self._record = result._record
        return self
    ...
    def FromPrimary(cls: Type[R], connection, pkey_value) -> R:
        ...
        if not record:
            raise NotExistError(
                "There is no %r for primary key %r" % (cls.__name__, pkey_value)
            )
    ...
    def _GetSearchQuery(cls, connection, tables, search):
        ...
        for column in cls.SEARCHABLE_COLUMNS:
            ...
            if othertable_name != thistable and othertable_name not in tables:
                ...
                conditions.append(
                    "`%s`.`%s` = `%s`.`%s`"
                    % (thistable, othertable_name, othertable_name, key)
                )
                tables.append(othertable_name)
            searchconditions.append("`%s`.`%s` %s" % (othertable_name, column, like))
...
class VersionedRecord(Record):
    ...
    def FromIdentifier(cls, connection, identifier):
        ...
        with connection as cursor:
            record = cursor.Select(
                table=cls.TableName(),
                order=[(cls._PRIMARY_KEY, True)],
                conditions="`%s`=%s" % (cls.RecordKey(), safe_id),
                limit=1,
            )
        if not record:
            raise NotExistError(
                "There is no %r for identifier %r" % (cls.__name__, identifier)
            )
        return cls(connection, record[0])
    ...
    def Versions(cls, connection, identifier, conditions="1"):
        ...
        with connection as cursor:
            records = cursor.Select(
                table=cls.TableName(),
                conditions="`%s` = %s AND %s" % (cls.RecordKey(), safe_id, conditions),
            )
    ...
    def _MaxRecordKey(cls, cursor):
        ...
        last_key = cursor.Select(
            table=cls.TableName(),
            fields=cls.RecordKey(),
            order=[(cls.RecordKey(), True)],
            limit=1,
        )
        if last_key:
...
class CachedPage(object):
    ...
    def Clean(cls, connection, maxage=None):
        ...
        where
          ...
          """
                % (cls.TableName(), (cls.MAXAGE if maxage is None else maxage))
            )
