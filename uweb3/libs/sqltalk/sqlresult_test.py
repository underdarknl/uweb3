...
class ResultRowDataExtraction(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.values = ("Elmer", "Underdark", 24, "python", "geek", "Ubuntu")
    ...
    def testGetItems(self):
        ...
        self.assertEquals(self.row.items(), zip(self.names, self.values))
        self.assertEquals(dict(self.row.items()), dict(zip(self.names, self.values)))
...
class ResultSetBasicOperation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.result = tuple(
            tuple(2**i for i in range(j, j + 4)) for j in range(0, 13, 4)
        )

    def testFalseWhenEmpty(self):
        """ResultSet is boolean False when there's all but a result."""
        result = sqlresult.ResultSet(
