...
# custom modules
from uweb3.libs.safestring import (
    SQLSAFE,
    Basesafestring,
...
class TestHTMLStringMethods(unittest.TestCase):
    ...
    def test_format_nested_safesametype(self):
        testdata = HTMLsafestring("test {} {}").format("<b>", HTMLsafestring("<b>"))
    ...
    def test_join(self):
        ...
        testdata = HTMLsafestring("").join(
            (HTMLsafestring("<b>"), HTMLsafestring("<b>"))
        )
    ...
    def test_join_unsafe(self):
        ...
        testdata = HTMLsafestring("").join(("<b>", HTMLsafestring("<b>")))
        self.assertEqual(testdata, "&lt;b&gt;<b>")
        self.assertIsInstance(testdata, HTMLsafestring)
...
class TestURLsafestringMethods(unittest.TestCase):
    def test_headerinjection(self):
        ...
        testdata = URLsafestring(
            "https://underdark.nl/somefile\n\nlocation: http://attacker.nl", unsafe=True
        )
...
class TestEmailAddresssafestringMethods(unittest.TestCase):
    def test_unsafe_init(self):
        """See if we correctly clean up header injection attemps"""
        testdata = EmailAddresssafestring(
...
class TestSQLSAFEMethods(unittest.TestCase):
    def test_user_supplied_safe_value(self):
        user_supplied_safe_object = SQLSAFE(
            "SELECT * FROM users WHERE username = 'username\t'"
        )
        self.assertEqual(
            user_supplied_safe_object,
            "SELECT * FROM users WHERE username = 'username\t'",
        )
    ...
    def test_escaping(self):
        ...
        self.assertEqual(
            testdata, "SELECT * FROM users WHERE username = 'username\\\"'"
        )
        testdata = SQLSAFE(
    ...
    def test_concatenation(self):
        ...
        self.assertEqual(
            testdata + other,
            "SELECT * FROM users WHERE username = 'username\\'' AND firstname=\\'test\\'",
        )
        testdata = SQLSAFE(
            """SELECT * FROM users WHERE username = ?""",
            values=('username"',),
