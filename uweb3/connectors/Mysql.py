...
class Mysql(Connector):
    ...
    def __init__(self, config, options, request, debug=False):
        ...
        try:
            ...
            raise ConnectionError(
                'Connection to "%s" of type "%s" resulted in: %r'
                % (self.Name(), type(self), e)
            )
    ...
    def SSLConfig(self):
        ...
        if any(
            (
                self.options.get("ssl_ca"),
                self.options.get("ssl_key"),
                self.options.get("ssl_cert"),
            )
        ):
            ssl = {
                "ca": self.options.get("ssl_ca"),
    ...
    def Disconnect(self):
        ...
        if self.debug:
            print(
                "%s closed connection to: %r"
                % (self.Name(), self.options.get("database"))
            )
