...
class DaemonContext(object):
  ...
  @staticmethod
  def terminate(signal_number, _stack_frame):
    ...
    * Raise a ``SystemExit`` exception explaining the signal.
    """
    raise SystemExit('Terminating on signal %r' % signal_number)
...
def detach_process_context():
  ...
  def fork_then_exit_parent(error_message):
    ...
    try:
      ...
      raise DaemonProcessDetachError('%s: [%d] %s' % (
...
def is_socket(fd):
  ...
  try:
    ...
    if not exc_errno == errno.ENOTSOCK:
      # IF FALSE: Socket operation on non-socket
      # IF TRUE: Some other socket error
      return True
  return False
...
def is_detach_process_context_required():
  ...
  * Process was started by `inetd`.
  ...
  return not (is_process_started_by_init()
              or is_process_started_by_superserver())
...
def close_file_descriptor_if_open(fd):
  ...
  try:
    ...
    if exc.errno == errno.EBADF:
      ...
      raise DaemonOSEnvironmentError(
        'Failed to close file descriptor %d (%s)' % (fd, exc))
