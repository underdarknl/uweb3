...
class AlchemyBaseRecord:
        ...
        for key, value in record.items():
            if key not in self.__table__.columns.keys():
                raise AttributeError(
                    f"Key '{key}' not specified in class '{self.__class__.__name__}'"
                )
    ...
    def __eq__(self, other):
        ...
        for key in self.__table__.columns.keys():
            ...
            if isinstance(self, AlchemyBaseRecord) != isinstance(
                other, AlchemyBaseRecord
            ):
                # Only one of the two is a BaseRecord instance
                if (
                    isinstance(self, AlchemyBaseRecord)
                    and value.key != other_value
                    or isinstance(other, AlchemyBaseRecord)
    ...
    def iteritems(self):
        ...
        return chain(
            ...
            (
                (child[0], getattr(self, child[0]))
                for child in inspect(type(self)).relationships.items()
            ),
        )
    ...
    def itervalues(self):
        ...
        return chain(
            (getattr(self, key) for key in self.__table__.columns.keys()),
            (
                getattr(self, child[0])
                for child in inspect(type(self)).relationships.items()
            ),
        )
